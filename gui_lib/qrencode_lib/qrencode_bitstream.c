...
BitStream *BitStream_new(void)
{
	...
	bstream = (BitStream *)qrencode_malloc(sizeof(BitStream));
	if(bstream == NULL) return NULL;

	bstream->length = 0;
	bstream->data = (unsigned char *)qrencode_malloc(DEFAULT_BUFSIZE);
	if(bstream->data == NULL) {
		qrencode_free(bstream);
		return NULL;
	}
...
BitStream *BitStream_newWithBits(size_t size, unsigned char *bits)
{
	...
	bstream->data = (unsigned char *)qrencode_malloc(size);
	if(bstream->data == NULL) {
		qrencode_free(bstream);
		return NULL;
	}
...
static int BitStream_expand(BitStream *bstream)
{
	...
	data = (unsigned char *)qrencode_malloc(bstream->datasize * 2);
	if(data == NULL) {
		return -1;
	}
	memcpy(data,bstream->data,bstream->datasize);
	qrencode_free(bstream->data);
...
unsigned char *BitStream_toByte(BitStream *bstream)
{
	...
	data = (unsigned char *)qrencode_malloc((size + 7) / 8);
	if(data == NULL) {
		return NULL;
	}
