...
#include "qrencode.h"
#include "qrencode_config.h"
#include "qrencode_qrspec.h"
#include "qrencode_mqrspec.h"
#include "qrencode_bitstream.h"
#include "qrencode_qrinput.h"
#include "qrencode_rsecc.h"
#include "qrencode_split.h"
#include "qrencode_mask.h"
#include "qrencode_mmask.h"
...
void* qrencode_malloc(unsigned int size)
{
    return os_mem_alloc(RAM_TYPE_BUFFER_ON, size);
}
void  qrencode_free(void* ptr)
{
    os_mem_free(ptr);
}
...
static QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw = (QRRawCode *)qrencode_malloc(sizeof(QRRawCode));
	...
	raw->ecccode = (unsigned char *)qrencode_malloc((size_t)raw->eccLength);
	if(raw->ecccode == NULL) {
		qrencode_free(raw->datacode);
		qrencode_free(raw);
		return NULL;
	}

	raw->blocks = QRspec_rsBlockNum(spec);
	raw->rsblock = (RSblock *)qrencode_malloc((size_t)(raw->blocks)*sizeof(RSblock));
...
static void QRraw_free(QRRawCode *raw)
{
	if(raw != NULL) {
		qrencode_free(raw->datacode);
		qrencode_free(raw->ecccode);
		qrencode_free(raw->rsblock);
		qrencode_free(raw);
	}
}
...
static MQRRawCode *MQRraw_new(QRinput *input)
{
	...
	raw = (MQRRawCode *)qrencode_malloc(sizeof(MQRRawCode));
	if(raw == NULL) return NULL;
	...
	raw->rsblock = (RSblock *)qrencode_malloc(sizeof(RSblock));
	if(raw->rsblock == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
...
static QRcode *QRcode_new(int version, int width, unsigned char *data)
{
	...
	qrcode = (QRcode *)qrencode_malloc(sizeof(QRcode));
...
{
	if(qrcode != NULL) {
		qrencode_free(qrcode->data);
		qrencode_free(qrcode);
	}
}
...
static QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	if(mask == -2) { // just for debug purpose
		masked = (unsigned char *)qrencode_malloc((size_t)(width * width));
...
static QRcode_List *QRcode_List_newEntry(void)
{
	...
	entry = (QRcode_List *)qrencode_malloc(sizeof(QRcode_List));
...
unsigned int QRcode_get_width(unsigned char* data, QRecLevel qr_lvl)
{
    unsigned int width = 0;
    QRcode *qrcode_data = QRcode_encodeString(data, 1, qr_lvl, QR_MODE_8, 1);
    if(qrcode_data != NULL) {
        width = qrcode_data->width;
