...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	entry = (QRinput_List *)qrencode_malloc(sizeof(QRinput_List));
	...
	entry->data = NULL;
	if(size > 0) {
		entry->data = (unsigned char *)qrencode_malloc((size_t)size);
		if(entry->data == NULL) {
			qrencode_free(entry);
			return NULL;
		}
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
	...
	n = (QRinput_List *)qrencode_malloc(sizeof(QRinput_List));
	if(n == NULL) return NULL;

	n->mode = entry->mode;
	n->size = entry->size;
	n->data = (unsigned char *)qrencode_malloc((size_t)n->size);
	if(n->data == NULL) {
		qrencode_free(n);
		return NULL;
	}
...
QRinput *QRinput_new2(int version, QRecLevel level)
{
	...
	input = (QRinput *)qrencode_malloc(sizeof(QRinput));
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
	...
	entry = (QRinput_InputList *)qrencode_malloc(sizeof(QRinput_InputList));
	if(entry == NULL) return NULL;
...
QRinput_Struct *QRinput_Struct_new(void)
{
	...
	s = (QRinput_Struct *)qrencode_malloc(sizeof(QRinput_Struct));
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
	...
	data = (unsigned char *)qrencode_malloc((size_t)bytes);
